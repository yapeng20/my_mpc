...
system(['sumo-gui -c ' ' C:\Users\LYP\Desktop\my_mpc\sumo_control_jiaotongliu\test.sumocfg ' '--remote-port 8873 --start&']);
...
%     end
    ...
    if d < 8000
        if VLimit(1,1) == VLimit(1,2)
            tempTargetSpeed = VLimit(1,2); 
%             traci.vehicle.setSpeed('0', VLimit(1,2));
        else
            tempTargetSpeed =  x0(2,1); 
%             traci.vehicle.setSpeed('0', x0(2,1));
        end
    else
        tempTargetSpeed =  v_max(end); 
%        traci.vehicle.setSpeed('0', v_max(end)); 
    end
    targetSpeed = [targetSpeed;tempTargetSpeed;];
    traci.vehicle.setSpeed('0', tempTargetSpeed);
...
%     t0 = t0 + 1;
    ...
    counta =[counta;a]; 
     ...
     vehiclePosition= [vehiclePosition;0;];
    vehicleSpeed= [vehicleSpeed;0;];
    vehicleDistance= [vehicleDistance;0;];   
    counta =[counta;0]; 
    t = [t;t0];
    t0 = t0 + 1;
    end
...
%%%%%%%绘制位移图1
open light_jiaotongliu.fig;
% legend on;
hold on; 
h5 = plot(t,vehicleDistance,'Color',[1 0 0],'LineWidth',1.5);
% legend([h1 h5],'被控车辆位移轨迹');
% a_1031 = get(h5,propertyArray);
% a_1030 = get(0,'FigureColor'); 
% get
hold off;
%%%%%%%绘制位移图2
open distance_jiaotongliu.fig;
hold on; 
plot(t,vehicleDistance,'-','Color',[1 0 0],'LineWidth',3);
%%%%%%%绘制速度图
open speed_jiaotongliu.fig;
hold on; 
plot(t,vehicleSpeed'.*3.6,'-','Color',[1 0 0],'LineWidth',3);
%%%%%%%绘制加速度图
open a_jiaotongliu.fig;
hold on; 
plot(t,counta,'-','Color',[1 0 0],'LineWidth',3);
%%%%%%%绘制需求扭矩图
open F_jiaotongliu.fig;
hold on; 
plot(t,targetF,'-','Color',[1 0 0],'LineWidth',3);
%%%%%%%绘制需求能量图
open W_jiaotongliu.fig;
hold on; 
